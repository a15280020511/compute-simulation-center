...
Ticket factor levels are requested targets only. They cannot increase achieved
credibility without runtime facts or immutable typed evidence.
"""
...
def _derived_assumption_level(assurance: Mapping[str, Any]) -> int:
    ...
    if assurance.get("status") == "WARN":
        return 1
    calibrated = all(
        row.get("calibration_status") in {"calibrated", "validated"}
    ...
    if calibrated:
        return 4
    falsifiable = all(
        row.get("falsification_test") and row.get("invalid_when") for row in rows
    )
    return 3 if falsifiable else 2


def _mechanism_assurance(ticket: Mapping[str, Any]) -> dict[str, Any]:
    raw_rows = ticket.get("mechanism_register")
...
def _validate_profile(raw: Any) -> dict[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("credibility_profile must be an object")
...
def _evidence_levels(
    profile: Mapping[str, Any], factors: set[str]
    ...
    for row in profile.get("evidence") or []:
        ...
        level = EVIDENCE_LEVEL.get(evidence_type, 1)
        if evidence_type == "independent_check" and row.get("independent") is not True:
            level = 1
        for raw_factor_id in row.get("factor_ids", []):
            factor_id = str(raw_factor_id)
            if factor_id not in factors:
                raise ValueError(
                    f"credibility evidence references unknown factor: {factor_id}"
                )
            counts[factor_id] += 1
...
def _derived_levels(
    ...
    derived = {
        ...
        "conceptual_model": int(mechanism["derived_conceptual_model_level"]),
    ...
    if isinstance(data_context, Mapping) and data_context.get("variables"):
...
def _required_levels(
    ...
    configuration = registry["profiles"][decision_class]
    required = {
        factor_id: int(
            configuration["factor_overrides"].get(
                factor_id, configuration["default_required_level"]
            )
        )
        for factor_id in factor_ids
    }
    if profile.get("model_influence") != "high" or profile.get("decision_consequence") != "high":
        return required
    escalation = registry["risk_escalation"][
        "high_model_influence_and_high_consequence"
    ]
    for factor_id in escalation["factors"]:
        required[factor_id] = min(
            int(escalation["cap"]),
            required[factor_id] + int(escalation["increment"]),
        )
    return required
...
def _factor_assessments(
    ...
    evidence_levels: Mapping[str, int],
    ...
    for factor_id, metadata in factors.items():
        derived_level = int(derived.get(factor_id, 0))
        evidence_level = int(evidence_levels.get(factor_id, 0))
        achieved = max(derived_level, evidence_level)
        threshold = int(required[factor_id])
        item = {
            "factor_id": factor_id,
            ...
            "typed_evidence_level": evidence_level,
            "required_level": threshold,
            "evidence_count": int(evidence_counts[factor_id]),
            "status": "PASS" if achieved >= threshold else "GAP",
        }
        rows.append(item)
        if item["status"] == "GAP":
            gaps.append(item)
...
def _case_status(
    ...
    critical_mechanism_block = (
        decision_class == "high_stakes" and mechanism["critical_gap_count"] > 0
    )
    if decision_class == "high_stakes" and (
        not profile or gaps or critical_mechanism_block
    ):
        return "BLOCKED"
    if gaps or mechanism["critical_gap_count"]:
        return "WARN"
    return "PASS"
...
def build_credibility_case(
    ...
    profile = _validate_profile(ticket.get("credibility_profile"))
    requested = {
        str(key): int(value)
        for key, value in (profile.get("factor_levels") or {}).items()
    }
    ...
    required = _required_levels(registry, decision_class, set(factors), profile)
    rows, gaps = _factor_assessments(
        factors,
        requested,
        derived,
        evidence_levels,
    ...
    weakest = (
        min(
            rows,
            key=lambda item: (
                item["achieved_level"] - item["required_level"],
                item["factor_id"],
            ),
        )
        if rows
    ...
    return {
        ...
        "status": _case_status(decision_class, profile, gaps, mechanism),
        "decision_class": decision_class,
        "model_id": str(model.get("model_id") or ""),
        "use_statement": str(
            profile.get("use_statement") or ticket.get("objective") or ""
        ),
        ...
        "note": (
            "Achieved credibility is derived from runtime facts and typed hashed "
            "evidence; ticket declarations cannot self-certify a level."
        ),
    }
