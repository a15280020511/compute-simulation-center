...
def gis_spatial_analysis(inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if mode == "geodesic_distance_matrix":
    ...
    if mode == "spatial_predicate_matrix":
    ...
    raise ComputeError(
        "inputs.mode must be geodesic_distance_matrix, transform_coordinates, "
        "geometry_overlay, spatial_predicate_matrix, or nearest_features"
    )
...
def bayesian_inference(inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if mode == "normal_mean_known_variance":
    ...
    if mode == "bayesian_linear_regression":
        x = _numeric_matrix(
            inputs.get("x"),
            "inputs.x",
            MAX_REGRESSION_ROWS,
        ...
        if n <= k:
            raise ComputeError(
                "bayesian_linear_regression requires more rows than coefficients"
            )
        prior_mean_raw = inputs.get("prior_mean")
        prior_mean = (
            np.zeros(k, dtype=float)
            if prior_mean_raw is None
            else _numeric_vector(
                prior_mean_raw, "inputs.prior_mean", k
            )
        )
        if prior_mean.size != k:
            raise ComputeError(
                "prior_mean length must match coefficient count"
            )
        prior_precision_value = inputs.get("prior_precision", 1e-6)
        if isinstance(prior_precision_value, (int, float)) and not isinstance(
            prior_precision_value, bool
        ):
            scalar = _positive(
                prior_precision_value, "inputs.prior_precision"
            )
            prior_precision = np.eye(k) * scalar
        else:
            prior_precision = _numeric_matrix(
                prior_precision_value,
                "inputs.prior_precision",
                k,
                k,
            )
            if prior_precision.shape != (k, k):
                raise ComputeError(
                    "prior_precision matrix must be square and match coefficient count"
                )
            if not np.allclose(
                prior_precision, prior_precision.T, atol=1e-10
            ):
                raise ComputeError("prior_precision must be symmetric")
            if np.min(np.linalg.eigvalsh(prior_precision)) <= 0:
                raise ComputeError(
                    "prior_precision must be positive definite"
                )
        prior_shape = _positive(
            inputs.get("prior_shape", 1e-3), "inputs.prior_shape"
        )
        prior_scale = _positive(
            inputs.get("prior_scale", 1e-3), "inputs.prior_scale"
        )
        posterior_precision = prior_precision + x.T @ x
        posterior_covariance_factor = np.linalg.inv(posterior_precision)
        posterior_mean = posterior_covariance_factor @ (
        ...
        coefficient_scale = np.sqrt(
            np.diag(
                (posterior_scale / posterior_shape)
                * posterior_covariance_factor
            )
        )
        alpha = (1.0 - credibility) / 2.0
        quantile = float(
            stats.t.ppf(1.0 - alpha, degrees_freedom)
        )
        names_raw = inputs.get("coefficient_names")
        if names_raw is None:
            names = (["intercept"] if add_intercept else []) + [
                f"x{index + 1}"
                for index in range(
                    x.shape[1] - (1 if add_intercept else 0)
                )
            ]
        else:
            names = [
                ...
                for value in _sequence(
                    names_raw, "inputs.coefficient_names"
                )
            ]
            ...
            coefficients.append(
                {
                    ...
                    "posterior_scale": float(coefficient_scale[index]),
                    "credible_interval": {
                        "level": credibility,
                        "lower": float(
                            posterior_mean[index]
                            - quantile * coefficient_scale[index]
                        ),
                        "upper": float(
                            posterior_mean[index]
                            + quantile * coefficient_scale[index]
        ...
        if "x_new" in inputs:
            ...
            if x_new.shape[1] != k:
                raise ComputeError(
                    "x_new column count must match fitted design"
                )
            for index, row in enumerate(x_new):
                ...
                scale = math.sqrt(
                    float(
                        ...
                        * (
                            1.0
                            + row @ posterior_covariance_factor @ row
                        )
                    )
                )
                predictive.append(
        ...
        sigma2_mean = (
            ...
            else posterior_scale / (posterior_shape - 1.0)
        )
        return {
            "mode": mode,
            "observations": n,
            "coefficients": coefficients,
            "posterior_noise_variance_mean": (
                None if sigma2_mean is None else float(sigma2_mean)
            ),
    ...
    raise ComputeError(
        "inputs.mode must be beta_binomial, gamma_poisson, "
        "normal_mean_known_variance, or bayesian_linear_regression"
    )
...
def econometric_analysis(inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if mode == "iv_2sls":
        ...
        exogenous_raw = inputs.get("exogenous")
        endogenous = _numeric_matrix(
            inputs.get("endogenous"),
            "inputs.endogenous",
            MAX_REGRESSION_ROWS,
            MAX_REGRESSION_COLUMNS,
        )
        instruments = _numeric_matrix(
            inputs.get("instruments"),
            "inputs.instruments",
            MAX_REGRESSION_ROWS,
            MAX_REGRESSION_COLUMNS,
        )
        if endogenous.shape[0] != y.size or instruments.shape[0] != y.size:
            raise ComputeError(
                "endogenous and instruments row counts must match y"
            )
        exogenous = (
            np.empty((y.size, 0))
            if exogenous_raw is None
            else _numeric_matrix(
                exogenous_raw,
                "inputs.exogenous",
                MAX_REGRESSION_ROWS,
                MAX_REGRESSION_COLUMNS,
        ...
        add_intercept = bool(inputs.get("add_intercept", True))
        base = (
            np.column_stack([np.ones(y.size), exogenous])
            if add_intercept
            else exogenous
        )
        x = np.column_stack([base, endogenous])
        z = np.column_stack([base, instruments])
        if x.shape[1] > z.shape[1]:
            raise ComputeError(
                "model is underidentified: instruments must cover endogenous regressors"
            )
        ...
        if np.linalg.matrix_rank(x_pz_x) < x.shape[1]:
            raise ComputeError("2SLS normal matrix is rank deficient")
        bread = np.linalg.inv(x_pz_x)
        ...
        if df <= 0:
            raise ComputeError(
                "iv_2sls requires more rows than coefficients"
            )
        covariance_upper = covariance_type.upper()
        if covariance_upper == "NONROBUST":
            sigma2 = float(residuals @ residuals / df)
            covariance = sigma2 * bread
        elif covariance_upper in {"HC0", "HC1"}:
            projected_x = z @ ztz_inv @ z.T @ x
            meat = projected_x.T @ (
                projected_x * (residuals**2)[:, None]
            )
            covariance = bread @ meat @ bread
            if covariance_upper == "HC1":
                covariance *= n / df
        else:
            raise ComputeError(
                "iv_2sls covariance_type must be nonrobust, HC0, or HC1"
            )
        standard_errors = np.sqrt(
            np.maximum(np.diag(covariance), 0.0)
        )
        ...
        exog_count = base.shape[1]
        raw_names = inputs.get("coefficient_names")
        if raw_names is None:
            names = (["intercept"] if add_intercept else []) + [
                f"exog_{index + 1}"
                for index in range(exogenous.shape[1])
            ] + [
                f"endogenous_{index + 1}"
                for index in range(endogenous.shape[1])
            ]
        else:
            names = [
                str(value)
                for value in _sequence(
                    raw_names, "inputs.coefficient_names"
                )
            ]
            if len(names) != k or len(set(names)) != k:
                raise ComputeError(
            ...
            coefficients.append(
                {
                    ...
                    "confidence_interval": {
                        ...
                        "lower": float(
                            beta[index]
                            - critical * standard_errors[index]
                        ),
                        "upper": float(
                            beta[index]
                            + critical * standard_errors[index]
        ...
        for column in range(endogenous.shape[1]):
            ...
            unrestricted = _regression_result(
                target,
                z,
                [f"z{index}" for index in range(z.shape[1])],
                weights=None,
                covariance_type="NONROBUST",
                confidence=confidence,
            )
            restricted_x = base
            restricted = _regression_result(
                target,
                restricted_x,
                [f"base{index}" for index in range(restricted_x.shape[1])],
                weights=None,
                covariance_type="NONROBUST",
                confidence=confidence,
            )
            ssr_u = (
                unrestricted["residual_standard_error"] ** 2
                * unrestricted["degrees_freedom_residual"]
            )
            ssr_r = (
                restricted["residual_standard_error"] ** 2
                * restricted["degrees_freedom_residual"]
            )
        ...
        return {
            "mode": mode,
            "observations": n,
            "covariance_type": covariance_upper,
            "coefficients": coefficients,
            "first_stage": first_stage,
            "identification_warning": (
                "Instrument validity requires relevance and exclusion restrictions; "
                "the first-stage statistic does not prove exogeneity."
            ),
            "exogenous_columns": exog_count,
            "endogenous_columns": endogenous.shape[1],
            "instrument_columns": instruments.shape[1],
        }

    raise ComputeError(
        "inputs.mode must be ols, wls, difference_in_differences, or iv_2sls"
    )
