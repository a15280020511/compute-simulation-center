#!/usr/bin/env python3
"""Policy-optimal dynamic orchestration for bounded linear/mixed-integer optimization."""
...
from dynamic_optimization_adapters import install_optimization_adapters
from operation_validation import validate_operation_inputs
from pipeline_adapters import ADAPTERS, PipelineAdapterError

install_optimization_adapters()

HERE = Path(__file__).resolve().parent
POLICY_PATH = HERE / "dynamic-optimization-policy.json"
GRAPH_PATH = HERE / "dynamic-optimization-capability-graph.json"
CONTRACT_PATH = HERE / "dynamic-optimization-stage-contracts.json"
FAMILY = "optimization"
DECLARED_OPERATION = "finance_decision_analysis"
DECLARED_MODE = "mixed_integer_optimization"
REQUIRED_STAGE_IDS = ("primary_optimization",)
RESULT_STAGE_ID = "primary_optimization"


class DynamicOptimizationError(ValueError):
    pass
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise DynamicOptimizationError(f"JSON root must be an object: {path.name}")
    return value


def _mapping(value: Any, name: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise DynamicOptimizationError(f"{name} must be an object")
    return value


def _sequence(value: Any, name: str) -> Sequence[Any]:
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        raise DynamicOptimizationError(f"{name} must be an array")
    return value


def _finite(value: Any, name: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise DynamicOptimizationError(f"{name} must be a finite number")
    result = float(value)
    if not math.isfinite(result):
        raise DynamicOptimizationError(f"{name} must be finite")
    return result
...
def _load_contracts() -> dict[str, Any]:
    ...
    if value.get("schema_version") != "compute-dynamic-optimization-stage-contracts-v1":
        raise DynamicOptimizationError("invalid optimization stage contract schema")
    if value.get("status") != "controlled-preview" or value.get("family") != FAMILY:
        raise DynamicOptimizationError("optimization stage contract identity mismatch")
    contracts = value.get("contracts")
    expected = {"primary_optimization", "independent_relaxation", "relaxation_bound_audit", "external_objective_benchmark"}
    if not isinstance(contracts, Mapping) or set(contracts) != expected:
        raise DynamicOptimizationError("optimization contracts must exactly cover admitted stages")
    normalized: dict[str, Any] = {}
    for stage_id, schema in contracts.items():
        if not isinstance(schema, Mapping):
            raise DynamicOptimizationError(f"invalid optimization contract: {stage_id}")
...
def _validate_stage_output(stage_id: str, result: Mapping[str, Any], contracts: Mapping[str, Any]) -> None:
    ...
    if not isinstance(schema, Mapping):
        raise DynamicOptimizationError(f"no optimization output contract for stage: {stage_id}")
    ...
    if errors:
        ...
        raise DynamicOptimizationError(f"optimization output contract failed for {stage_id} at {path}: {error.message}")


def _load_policy() -> dict[str, Any]:
    policy = _load_json(POLICY_PATH)
    expected = {
        "schema_version": "compute-dynamic-optimization-policy-v1",
    ...
    for key, expected_value in expected.items():
        if policy.get(key) != expected_value:
            raise DynamicOptimizationError(f"unsafe optimization policy: {key}")
    if policy.get("allowed_operations") != ["finance_decision_analysis"]:
        raise DynamicOptimizationError("optimization allowed_operations mismatch")
    adapters = policy.get("allowed_adapters")
    if not isinstance(adapters, list) or len(adapters) != len(set(adapters)):
        raise DynamicOptimizationError("optimization allowed_adapters must be unique")
    ...
    if solver.get("require_optimal_status") is not True or int(solver.get("num_search_workers") or 0) != 1:
        raise DynamicOptimizationError("optimization stage selector must require OPTIMAL with one worker")
    if not 0 < float(solver.get("max_time_seconds") or 0) <= 10:
        raise DynamicOptimizationError("optimization selector time bound is invalid")
    rules = _mapping(_mapping(policy.get("selection_policy"), "selection_policy").get("stage_rules"), "stage_rules")
    expected_rules = ["independent_relaxation", "relaxation_bound_audit", "external_objective_benchmark"]
    if list(rules) != expected_rules:
        raise DynamicOptimizationError("optimization optional rule order is fixed")
    for stage_id, raw_rule in rules.items():
        ...
        if isinstance(penalty, bool) or not isinstance(penalty, int) or penalty < 0:
            raise DynamicOptimizationError(f"invalid optimization penalty: {stage_id}")
        ...
        if any(isinstance(value, bool) or not isinstance(value, int) for value in benefits.values()):
            raise DynamicOptimizationError(f"invalid optimization benefits: {stage_id}")
        for name in ("eligible_all", "required_if_any"):
            values = rule.get(name, [])
            if not isinstance(values, list) or any(not isinstance(item, str) or not item for item in values):
                raise DynamicOptimizationError(f"{name} must be a string array: {stage_id}")
        coupled = rule.get("coupled_equal_to")
        if coupled is not None and coupled not in expected_rules:
            raise DynamicOptimizationError(f"unknown optimization coupling target: {coupled}")
    return policy


def _load_graph(policy: Mapping[str, Any]) -> dict[str, Any]:
    graph = _load_json(GRAPH_PATH)
    if graph.get("schema_version") != "compute-dynamic-optimization-capability-graph-v1":
        raise DynamicOptimizationError("invalid optimization graph schema")
    if graph.get("status") != "controlled-preview" or graph.get("family") != FAMILY:
        raise DynamicOptimizationError("optimization graph identity mismatch")
    if graph.get("selection_engine") != "ortools-cp-sat" or graph.get("graph_engine") != "networkx":
        raise DynamicOptimizationError("optimization graph engine mismatch")
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise DynamicOptimizationError(f"unsafe optimization graph policy: {key}")
    order = [str(item) for item in _sequence(graph.get("node_order"), "graph.node_order")]
    expected_order = ["primary_optimization", "independent_relaxation", "relaxation_bound_audit", "external_objective_benchmark"]
    raw_nodes = _mapping(graph.get("nodes"), "graph.nodes")
    if order != expected_order or set(raw_nodes) != set(expected_order):
        raise DynamicOptimizationError("optimization node order is fixed")
    ...
    for stage_id in order:
        ...
        if node.get("operation") not in allowed_operations:
            raise DynamicOptimizationError(f"optimization operation not allowlisted: {stage_id}")
        adapter = str(node.get("adapter") or "")
        if adapter not in allowed_adapters or adapter not in ADAPTERS:
            raise DynamicOptimizationError(f"optimization adapter not allowlisted: {adapter}")
        nodes[stage_id] = node
    if nodes["primary_optimization"].get("required") is not True or nodes["primary_optimization"].get("result_required") is not True:
        raise DynamicOptimizationError("primary optimization must be required result stage")
    for stage_id in order[1:]:
        if nodes[stage_id].get("required") is True or nodes[stage_id].get("result_required") is True:
            raise DynamicOptimizationError(f"optional optimization node marked required: {stage_id}")
    edges: list[tuple[str, str]] = []
    for raw_edge in _sequence(graph.get("precedence"), "graph.precedence"):
        edge = _sequence(raw_edge, "graph.precedence[]")
        if len(edge) != 2:
            raise DynamicOptimizationError("optimization precedence edge requires two nodes")
        edges.append((str(edge[0]), str(edge[1])))
    expected_edges = {
        ("primary_optimization", "independent_relaxation"),
        ("primary_optimization", "relaxation_bound_audit"),
        ("independent_relaxation", "relaxation_bound_audit"),
        ("primary_optimization", "external_objective_benchmark"),
    }
    if set(edges) != expected_edges:
        raise DynamicOptimizationError("optimization DAG does not match controlled branch-and-join structure")
    full = nx.DiGraph()
    ...
    if not nx.is_directed_acyclic_graph(full) or max(dict(full.out_degree()).values()) < 2:
        raise DynamicOptimizationError("optimization graph must be an acyclic branching DAG")
    ...
    if list(nx.lexicographical_topological_sort(full, key=lambda node: index[node])) != order:
        raise DynamicOptimizationError("optimization policy order disagrees with NetworkX topology")
...
def _signals(ticket: Mapping[str, Any]) -> tuple[dict[str, bool], dict[str, Any]]:
    if resolve_dynamic_family(ticket) != FAMILY:
        raise DynamicOptimizationError("ticket was not routed to optimization family")
    inputs = _mapping(ticket.get("inputs"), "ticket.inputs")
    if str(inputs.get("mode") or "") != DECLARED_MODE:
        raise DynamicOptimizationError("optimization family entry mode mismatch")
    variables = [_mapping(row, "inputs.variables[]") for row in _sequence(inputs.get("variables"), "inputs.variables")]
    constraints = [_mapping(row, "inputs.constraints[]") for row in _sequence(inputs.get("constraints", []), "inputs.constraints")]
    if not 1 <= len(variables) <= 200 or len(constraints) > 1000:
        raise DynamicOptimizationError("optimization problem size is outside governed bounds")
    names: set[str] = set()
    discrete = False
    relaxation_compatible = True
    for index, row in enumerate(variables):
        name = str(row.get("name") or "")
        if not name or name in names:
            raise DynamicOptimizationError("optimization variable names must be non-empty and unique")
        names.add(name)
        kind = str(row.get("type") or "continuous")
        if kind not in {"continuous", "integer", "binary"}:
            raise DynamicOptimizationError("optimization variable type must be continuous, integer, or binary")
        discrete = discrete or kind in {"integer", "binary"}
        lower = _finite(row.get("lower_bound", row.get("lower", 0.0)), f"variables[{index}].lower_bound")
        if abs(lower) > 1e-12:
            relaxation_compatible = False
        upper = row.get("upper_bound", row.get("upper"))
        if upper is not None:
            _finite(upper, f"variables[{index}].upper_bound")
        _finite(row.get("objective_coefficient", row.get("objective", 0.0)), f"variables[{index}].objective_coefficient")
    for index, row in enumerate(constraints):
        coefficients = _mapping(row.get("coefficients"), f"constraints[{index}].coefficients")
        if set(str(name) for name in coefficients) - names:
            raise DynamicOptimizationError("optimization constraint references unknown variable")
        for name, raw in coefficients.items():
            _finite(raw, f"constraints[{index}].{name}")
        _finite(row.get("rhs"), f"constraints[{index}].rhs")
        if str(row.get("relation") or "<=") not in {"<=", ">=", "=="}:
            raise DynamicOptimizationError("constraint relation must be <=, >=, or ==")
    context_raw = inputs.get("optimization_context")
    context = {} if context_raw is None else dict(_mapping(context_raw, "inputs.optimization_context"))
    crosscheck_requested = context.get("independent_relaxation_crosscheck", False)
    if not isinstance(crosscheck_requested, bool):
        raise DynamicOptimizationError("independent_relaxation_crosscheck must be boolean")
    if crosscheck_requested and not relaxation_compatible:
        raise DynamicOptimizationError("requested independent relaxation requires zero lower bounds")
    if "crosscheck_tolerance" in context:
        tolerance = _finite(context["crosscheck_tolerance"], "optimization_context.crosscheck_tolerance")
        if not 0 <= tolerance <= 1.0:
            raise DynamicOptimizationError("crosscheck_tolerance must be between 0 and 1")
    external_keys = {"external_objective_value", "external_objective_tolerance"}
    present = {key for key in external_keys if key in context}
    if present and present != external_keys:
        raise DynamicOptimizationError("external objective benchmark requires value and tolerance")
    external_available = present == external_keys
    if external_available:
        _finite(context["external_objective_value"], "optimization_context.external_objective_value")
        tolerance = _finite(context["external_objective_tolerance"], "optimization_context.external_objective_tolerance")
        if not 0 <= tolerance <= 1e9:
            raise DynamicOptimizationError("external objective tolerance is outside governed range")
    decision_class = _decision_class(ticket)
    signals = {
        "relaxation_compatible": relaxation_compatible,
        "crosscheck_requested": crosscheck_requested,
        "formal_or_high_stakes": decision_class in {"formal", "high_stakes"},
        "has_discrete_variables": discrete,
        "variable_count_ge_5": len(variables) >= 5,
...
def _solve(policy: Mapping[str, Any], graph: Mapping[str, Any], signals: Mapping[str, bool]) -> dict[str, Any]:
    ...
    if status != cp_model.OPTIMAL:
        raise DynamicOptimizationError(f"optimization stage selection must prove OPTIMAL; observed {solver.StatusName(status)}")
    ...
    if len(optional_ids) <= int(solver_policy["exhaustive_cross_check_max_optional_nodes"]):
        ...
        if not feasible:
            raise DynamicOptimizationError("no feasible optimization selections during exhaustive cross-check")
        ...
        if objective != best or selected not in optimal:
            raise DynamicOptimizationError(f"optimization CP-SAT optimum disagrees with exhaustive cross-check: solver={objective}, exhaustive={best}")
...
def plan_dynamic_optimization(ticket: Mapping[str, Any]) -> dict[str, Any]:
    policy = _load_policy()
    ...
    if len(selected_nodes) > int(policy["maximum_stages"]):
        raise DynamicOptimizationError("optimization plan exceeds maximum stages")
    ...
    if not nx.is_directed_acyclic_graph(runtime_graph):
        raise DynamicOptimizationError("selected optimization plan contains a cycle")
    ...
    if execution_order != expected_order:
        raise DynamicOptimizationError("NetworkX deterministic order disagrees with optimization policy order")
    ...
    try:
        for index, stage_id in enumerate(plan["stage_order"]):
            ...
            if operation not in operations or adapter_name not in ADAPTERS:
                raise DynamicOptimizationError(f"optimization handler or adapter unavailable at {stage_id}")
            for dependency in stage["depends_on"]:
                if dependency not in stage_results:
                    raise DynamicOptimizationError(f"optimization dependency has not completed: {dependency}")
            ...
            try:
                ...
                raise DynamicOptimizationError(f"adapter failed at {stage_id}: {exc}") from exc
            ...
            if not isinstance(result, Mapping):
                raise DynamicOptimizationError(f"stage returned non-object result: {stage_id}")
            result_dict = dict(result)
            _validate_stage_output(stage_id, result_dict, contracts)
            if stage_id in {"relaxation_bound_audit", "external_objective_benchmark"} and result_dict.get("status") != "PASS":
                raise DynamicOptimizationError(f"optimization validation failed at {stage_id}")
...
def run_dynamic_optimization_ticket(ticket: Mapping[str, Any], output_dir: Path, operations: Mapping[str, Callable[[Mapping[str, Any]], dict[str, Any]]]) -> dict[str, Any]:
    if resolve_dynamic_family(ticket) != FAMILY:
        raise DynamicOptimizationError("ticket is not an admitted optimization dynamic request")
    ...
    validation_results = {stage_id: stage_results[stage_id] for stage_id in ("independent_relaxation", "relaxation_bound_audit", "external_objective_benchmark") if stage_id in stage_results}
    ...
    (output_dir / "compute-summary.md").write_text(
        ...
        f"- Primary solver status: `{stage_results['primary_optimization']['status']}`\n"
        "- Network used: `false`\n"
        "- Model calls: `0`\n",
        encoding="utf-8",
    )
