#!/usr/bin/env python3
"""Policy-optimal dynamic orchestration for controlled distribution-drift analysis."""
...
from dynamic_drift_adapters import install_drift_adapters
...
install_drift_adapters()

HERE = Path(__file__).resolve().parent
POLICY_PATH = HERE / "dynamic-drift-policy.json"
GRAPH_PATH = HERE / "dynamic-drift-capability-graph.json"
CONTRACT_PATH = HERE / "dynamic-drift-stage-contracts.json"
FAMILY = "drift"
DECLARED_OPERATION = "finance_decision_analysis"
DECLARED_MODE = "evidently_data_drift"
REQUIRED_STAGE_IDS = ("distribution_drift",)
RESULT_STAGE_ID = "distribution_drift"
STAGE_ORDER = ["distribution_drift", "adwin_cross_check", "change_point_cross_check", "drift_share_audit"]


class DynamicDriftError(ValueError):
    pass
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise DynamicDriftError(f"JSON root must be an object: {path.name}")
    return value


def _mapping(value: Any, name: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise DynamicDriftError(f"{name} must be an object")
    return value


def _sequence(value: Any, name: str) -> Sequence[Any]:
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        raise DynamicDriftError(f"{name} must be an array")
    return value


def _finite(value: Any, name: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise DynamicDriftError(f"{name} must be numeric")
    result = float(value)
    if not math.isfinite(result):
        raise DynamicDriftError(f"{name} must be finite")
    return result
...
def _load_contracts() -> dict[str, Any]:
    ...
    if value.get("schema_version") != "compute-dynamic-drift-stage-contracts-v1":
        raise DynamicDriftError("invalid drift stage contract schema")
    if value.get("status") != "controlled-preview" or value.get("family") != FAMILY:
        raise DynamicDriftError("drift stage contract identity mismatch")
    contracts = value.get("contracts")
    if not isinstance(contracts, Mapping) or list(contracts) != STAGE_ORDER:
        raise DynamicDriftError("drift contracts must exactly cover admitted stages in fixed order")
    normalized: dict[str, Any] = {}
    for stage_id, schema in contracts.items():
        if not isinstance(schema, Mapping):
            raise DynamicDriftError(f"invalid drift contract: {stage_id}")
...
def _validate_stage_output(stage_id: str, result: Mapping[str, Any], contracts: Mapping[str, Any]) -> None:
    ...
    if not isinstance(schema, Mapping):
        raise DynamicDriftError(f"no drift output contract for stage: {stage_id}")
    ...
    if errors:
        ...
        raise DynamicDriftError(f"drift output contract failed for {stage_id} at {path}: {error.message}")


def _load_policy() -> dict[str, Any]:
    policy = _load_json(POLICY_PATH)
    expected = {
        "schema_version": "compute-dynamic-drift-policy-v1",
    ...
    for key, expected_value in expected.items():
        if policy.get(key) != expected_value:
            raise DynamicDriftError(f"unsafe drift policy: {key}")
    if policy.get("allowed_operations") != [DECLARED_OPERATION] or policy.get("allowed_entry_modes") != [DECLARED_MODE]:
        raise DynamicDriftError("drift operation/mode allowlist mismatch")
    adapters = policy.get("allowed_adapters")
    if not isinstance(adapters, list) or len(adapters) != len(set(adapters)):
        raise DynamicDriftError("drift allowed_adapters must be unique")
    ...
    if solver.get("require_optimal_status") is not True or int(solver.get("num_search_workers") or 0) != 1:
        raise DynamicDriftError("drift selector must require OPTIMAL with one worker")
    if not 0 < float(solver.get("max_time_seconds") or 0) <= 10:
        raise DynamicDriftError("drift selector time bound is invalid")
    rules = _mapping(_mapping(policy.get("selection_policy"), "selection_policy").get("stage_rules"), "stage_rules")
    expected_rules = STAGE_ORDER[1:]
    if list(rules) != expected_rules:
        raise DynamicDriftError("drift optional rule order is fixed")
    for stage_id, raw_rule in rules.items():
        ...
        if isinstance(penalty, bool) or not isinstance(penalty, int) or penalty < 0:
            raise DynamicDriftError(f"invalid drift penalty: {stage_id}")
        ...
        if any(isinstance(value, bool) or not isinstance(value, int) for value in benefits.values()):
            raise DynamicDriftError(f"invalid drift benefits: {stage_id}")
        for name in ("eligible_all", "required_if_any", "requires_selected"):
            values = rule.get(name, [])
            if not isinstance(values, list) or any(not isinstance(item, str) or not item for item in values):
                raise DynamicDriftError(f"{name} must be a string array: {stage_id}")
    return policy


def _load_graph(policy: Mapping[str, Any]) -> dict[str, Any]:
    graph = _load_json(GRAPH_PATH)
    if graph.get("schema_version") != "compute-dynamic-drift-capability-graph-v1":
        raise DynamicDriftError("invalid drift graph schema")
    if graph.get("status") != "controlled-preview" or graph.get("family") != FAMILY:
        raise DynamicDriftError("drift graph identity mismatch")
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise DynamicDriftError(f"unsafe drift graph policy: {key}")
    ...
    if order != STAGE_ORDER or set(raw_nodes) != set(STAGE_ORDER):
        raise DynamicDriftError("drift node order is fixed")
    ...
    if set(edges) != expected_edges:
        raise DynamicDriftError("drift DAG does not match controlled structure")
    full = nx.DiGraph()
    ...
    if not nx.is_directed_acyclic_graph(full) or max(dict(full.out_degree()).values()) < 2:
        raise DynamicDriftError("drift graph must be an acyclic branching DAG")
    ...
    if list(nx.lexicographical_topological_sort(full, key=lambda node: index[node])) != order:
        raise DynamicDriftError("drift policy order disagrees with NetworkX topology")
...
def _matrix_shape(value: Any, name: str) -> tuple[int, int]:
    ...
    if not 20 <= len(rows) <= 5000:
        raise DynamicDriftError(f"{name} must have 20 to 5000 rows")
    width: int | None = None
    for index, raw_row in enumerate(rows):
        ...
        if width is None:
            width = len(row)
        if len(row) != width:
            raise DynamicDriftError(f"{name} must be rectangular")
        for column, item in enumerate(row):
            _finite(item, f"{name}[{index}][{column}]")
    if width is None or not 2 <= width <= 30:
        raise DynamicDriftError(f"{name} must contain 2 to 30 columns")
...
def _signals(ticket: Mapping[str, Any]) -> tuple[dict[str, bool], dict[str, Any]]:
    if resolve_dynamic_family(ticket) != FAMILY:
        raise DynamicDriftError("ticket was not routed to drift family")
    inputs = _mapping(ticket.get("inputs"), "ticket.inputs")
    if str(inputs.get("mode") or "") != DECLARED_MODE:
        raise DynamicDriftError("drift entry mode mismatch")
    reference_rows, reference_columns = _matrix_shape(inputs.get("reference"), "inputs.reference")
    current_rows, current_columns = _matrix_shape(inputs.get("current"), "inputs.current")
    if reference_columns != current_columns:
        raise DynamicDriftError("reference and current column counts must match")
    context_raw = inputs.get("drift_context")
    context = {} if context_raw is None else dict(_mapping(context_raw, "inputs.drift_context"))
    allowed_context = {
        "adwin_cross_check_requested", "change_point_cross_check_requested",
        "expected_drift_share", "drift_share_tolerance", "adwin_delta",
        "change_point_cost_model", "change_point_penalty",
    }
    unexpected = sorted(set(context) - allowed_context)
    if unexpected:
        raise DynamicDriftError(f"drift_context contains unsupported fields: {unexpected}")
    for name in ("adwin_cross_check_requested", "change_point_cross_check_requested"):
        value = context.get(name, False)
        if not isinstance(value, bool):
            raise DynamicDriftError(f"{name} must be boolean")
    share_keys = {"expected_drift_share", "drift_share_tolerance"}
    present_share = {key for key in share_keys if key in context}
    if present_share and present_share != share_keys:
        raise DynamicDriftError("drift-share audit requires expected_drift_share and drift_share_tolerance")
    share_available = present_share == share_keys
    if share_available:
        expected = _finite(context["expected_drift_share"], "drift_context.expected_drift_share")
        tolerance = _finite(context["drift_share_tolerance"], "drift_context.drift_share_tolerance")
        if not 0.0 <= expected <= 1.0 or tolerance < 0:
            raise DynamicDriftError("expected_drift_share must be [0,1] and tolerance non-negative")
    if "adwin_delta" in context:
        delta = _finite(context["adwin_delta"], "drift_context.adwin_delta")
        if not 1e-8 <= delta <= 0.5:
            raise DynamicDriftError("adwin_delta is out of range")
    if "change_point_cost_model" in context and str(context["change_point_cost_model"]) not in {"l1", "l2", "rbf", "normal"}:
        raise DynamicDriftError("unsupported change_point_cost_model")
    if "change_point_penalty" in context and _finite(context["change_point_penalty"], "drift_context.change_point_penalty") < 0:
        raise DynamicDriftError("change_point_penalty must be non-negative")
    decision_class = _decision_class(ticket)
    signals = {
        "adwin_cross_check_requested": bool(context.get("adwin_cross_check_requested", False)),
        "change_point_cross_check_requested": bool(context.get("change_point_cross_check_requested", False)),
        "expected_drift_share_available": share_available,
    ...
    return signals, {
        ...
        "column_count": reference_columns,
        **signals,
    }
...
def _solve(policy: Mapping[str, Any], graph: Mapping[str, Any], signals: Mapping[str, bool]) -> dict[str, Any]:
    ...
    if status != cp_model.OPTIMAL:
        raise DynamicDriftError(f"drift stage selection must prove OPTIMAL; observed {solver.StatusName(status)}")
    ...
    if len(optional_ids) <= int(solver_policy["exhaustive_cross_check_max_optional_nodes"]):
        ...
        if not feasible:
            raise DynamicDriftError("no feasible drift selections during exhaustive cross-check")
        ...
        if objective != best or selected not in optimal:
            raise DynamicDriftError(f"drift CP-SAT optimum disagrees with exhaustive cross-check: solver={objective}, exhaustive={best}")
...
def plan_dynamic_drift(ticket: Mapping[str, Any]) -> dict[str, Any]:
    policy = _load_policy()
    ...
    if not nx.is_directed_acyclic_graph(runtime_graph):
        raise DynamicDriftError("selected drift plan contains a cycle")
    ...
    if execution_order != expected_order:
        raise DynamicDriftError("NetworkX deterministic order disagrees with drift policy order")
    ...
    return {
        ...
        "planning_reasons": [
            "drift family is selected only from finance_decision_analysis:evidently_data_drift with bounded reference/current matrices",
            ...
            "Evidently generates the governed multivariate drift asset and fixed per-column KS screening statistics",
            "fixed adapters select the maximum-KS column deterministically before River ADWIN and Ruptures consume the original ordered reference/current values",
            "benchmark_comparison validates an explicit expected KS-screen drift share without natural-language inference",
...
def _execute(ticket: Mapping[str, Any], operations: Mapping[str, Callable[[Mapping[str, Any]], dict[str, Any]]], output_dir: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]], list[dict[str, Any]], dict[str, float]]:
    plan = plan_dynamic_drift(ticket)
    ...
    try:
        for index, stage_id in enumerate(plan["stage_order"]):
            ...
            if operation not in operations or adapter_name not in ADAPTERS:
                raise DynamicDriftError(f"handler or adapter unavailable at {stage_id}")
            for dependency in stage["depends_on"]:
                if dependency not in stage_results:
                    raise DynamicDriftError(f"dependency has not completed: {dependency}")
            ...
            try:
                ...
                raise DynamicDriftError(f"adapter failed at {stage_id}: {exc}") from exc
            ...
            if not isinstance(result, Mapping):
                raise DynamicDriftError(f"stage returned non-object result: {stage_id}")
            result_dict = dict(result)
            _validate_stage_output(stage_id, result_dict, contracts)
            if stage_id == "drift_share_audit" and result_dict.get("status") != "PASS":
                raise DynamicDriftError("drift-share validation failed")
...
def run_dynamic_drift_ticket(ticket: Mapping[str, Any], output_dir: Path, operations: Mapping[str, Callable[[Mapping[str, Any]], dict[str, Any]]]) -> dict[str, Any]:
    if resolve_dynamic_family(ticket) != FAMILY:
        raise DynamicDriftError("ticket is not an admitted drift dynamic request")
    ...
    if "distribution_drift" in stage_results:
        software["evidently"] = _package_version("evidently")
    if "adwin_cross_check" in stage_results:
        software["river"] = _package_version("river")
    if "change_point_cross_check" in stage_results:
        software["ruptures"] = _package_version("ruptures")
    runtime_graph = nx.DiGraph()
    ...
    cross_engines = []
    if "adwin_cross_check" in stage_results:
        cross_engines.append("river-adwin")
    if "change_point_cross_check" in stage_results:
        cross_engines.append("ruptures-pelt")
    _write_json(output_dir / "compute-audit.json", {
        ...
        "graph_contains_branching": graph_contains_branching, "primary_engine": "evidently", "cross_check_engines": cross_engines,
