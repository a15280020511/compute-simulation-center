...
OPTIONAL_ENGINE_AVAILABLE = (
    importlib.util.find_spec("cvc5") is not None
    and importlib.util.find_spec("flint") is not None
)
MODULE = None
if OPTIONAL_ENGINE_AVAILABLE:
    SPEC = importlib.util.spec_from_file_location(
        "certified_math_task", HERE / "certified_math_task.py"
    )
...
@unittest.skipUnless(
    OPTIONAL_ENGINE_AVAILABLE,
    "certified math optional engine is validated in its isolated workflow",
)
class CertifiedMathTests(unittest.TestCase):
    def test_schema_is_valid(self) -> None:
        schema = json.loads(
            (HERE / "certified-math-ticket.schema.json").read_text(encoding="utf-8")
        )
    ...
    def test_exact_polynomial(self) -> None:
        assert MODULE is not None
        result = MODULE.exact_integer_polynomial(
            {"coefficients": [2, -3, 5], "x": 10}
        )
    ...
    def test_certified_interval_contains_rational(self) -> None:
        assert MODULE is not None
        result = MODULE.certified_rational_interval(
            {"numerator": 1, "denominator": 3, "precision_digits": 60}
        )
    ...
    def test_end_to_end_receipt_and_manifest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            receipt = json.loads(
                (output_dir / "certified-math-receipt.json").read_text(
                    encoding="utf-8"
                )
            )
            manifest = json.loads(
                (output_dir / "certified-math-manifest.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertEqual(
                receipt["status"], "COMPUTE_CERTIFIED_MATH_COMPLETED"
            )
